...
new Vue({
    el:"#supermarket",
   data:{
        getMarketDealWebList:window.dataUrl+"/web/deal/getMarketDealWebList",//合金超市订单列表
        getHouseList:window.dataUrl+"/web/deal/getHouseList",//合金超市订单列表
    ...
    mounted: function () {
        ...
        $(".weight").off().on("click", function () {
            if(onn){
	        	...
	        	upDownList.weight = 1
	        	upDownList.price = null
	        	upDownList.createTime = null
                $(".bg1").css({
                    "background-position":"0px 10px"
                })
                ajax({        //合金超市订单列表
                    url: _this.getMarketDealWebList,
                    type: "post",
...
//  		$('.classify').eq(0).click()	
		...
		//触发后更改样式与下级item与列表数据				
		 $('.super-right li a').off().click(function(ev){
		 	...
		 	}else if( $(ev.target).parent().parent().hasClass("super-right0") ){
		 		$('.super-right2 li a').css({background:'none',color: "#666"})
			 	   	_this.args_list.gradeId = null
			 		_this.updateList(type,typeId)
		 	}else{
            ...
            $(".smark").off().on("mouseover", function () {
                $(event.target).parent().siblings(".hover-fade").fadeIn(500)
            }).off().on("mouseout", function () {
...
//      }
    ...
    methods:  {
    	...
    	updateList(type,id){
    		...
    		}else if(type == 'houseId'){
    			this.args_list.houseId = id
    		}
    		    
    		    //更新列表
			    ajax({
                    url: that.getMarketDealWebList,
                    type: "post",
